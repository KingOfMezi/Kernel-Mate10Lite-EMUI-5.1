					...
					uint8_t flags)
{
	...
	TC_NS_Shared_MEM *temp_shared_mem = NULL;
	...
	for (i = 0; i < 4; i++) {
		...
		if ((TEEC_MEMREF_TEMP_INPUT == param_type) ||
			...
			temp_shared_mem = tc_mem_allocate(dev_file, buffer_size);
			/* If buffer size is zero or malloc failed */
			if (IS_ERR(temp_shared_mem)) {
			...
			local_temp_buffer[i].temp_buffer = temp_shared_mem->kernel_addr;
			local_temp_buffer[i].size = temp_shared_mem->len;
			local_temp_buffer[i].shared_mem = temp_shared_mem;
			...
			if ((TEEC_MEMREF_TEMP_INPUT == param_type) ||
				...
				/* Kernel side buffer */
				if (copy_from_client(temp_shared_mem->kernel_addr,
						     (void *)client_param->memref.
						     buffer, buffer_size,
						     kernel_params)) {
			...
			operation->params[i].memref.buffer = virt_to_phys(temp_shared_mem->kernel_addr);
			operation->buffer_h_addr[i] = virt_to_phys(temp_shared_mem->kernel_addr) >> 32;
			  ...
			  TC_NS_Temp_Buf local_temp_buffer[4])
{
	...
	for (i = 0; i < 4; i++) {
		...
		if ((TEEC_MEMREF_TEMP_INPUT == param_type) ||
			...
			if (IS_ERR(local_temp_buffer[i].shared_mem)) {
				tloge("The shared mem of local_temp_buffer is null\n");
				kfree(operation);
				return -EINVAL;
			}
			tc_mem_free(local_temp_buffer[i].shared_mem);
