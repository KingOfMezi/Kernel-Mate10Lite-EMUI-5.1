...
static int check_teecd_hash(void)
{
	...
	if (g_teecd_hash_enable) {
		if (tee_calc_task_hash(digest)
			|| memcmp(digest, teecd_hash, SHA256_DIGEST_LENTH)) { /*lint !e747*/
...
static int check_package_name(TC_NS_DEV_File *dev_file)
{
	...
	if (TEE_REQ_FROM_USER_MODE == dev_file->kernel_api) {
		...
		res = get_cmdline(current, real_cmdline, PAGE_SIZE); /*lint !e747*/
		if (!res || (strnlen(real_cmdline, res) != dev_file->pkg_name_len)) { /*lint !e747 !e732*/
