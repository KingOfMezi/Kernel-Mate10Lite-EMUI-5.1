					    ...
					    unsigned long event, void *data)
{
	...
	if (g_ts_kit_platform_data.feature_info.charger_info.charger_supported != 0) {
		if(TS_CHARGER_TYPE_NONE == event || TS_PLEASE_PROVIDE_POWER == event){
...
static void ts_kit_charger_notifier_register(void)
{
	int error;
	enum ts_charger_state charger_state = USB_PIUG_IN;
	enum ts_charger_type charger_type = TS_CHARGER_TYPE_NONE;
	...
	if (error < 0) {
		...
		if(TS_CHARGER_TYPE_NONE == charger_type || TS_PLEASE_PROVIDE_POWER == charger_type) {
			charger_state = USB_PIUG_OUT;
		} else {
