				...
				struct ts_fingers *info)
{
	...
	for (i = 0; i < touch_num; i++) {
		...
		info->fingers[id].major = touch_ewxy[2 * i];
		info->fingers[id].minor = touch_ewxy[2 * i + 1];
		info->fingers[id].pressure = w;
		info->fingers[id].status = TP_FINGER;
		cur_index |= 1 << id;
		TS_LOG_DEBUG("%s:[%d](%d, %d, %d,ewx = %d, ewy =  %d)\n", __func__, id, x, y, w,touch_ewxy[2 * i] , touch_ewxy[2 * i + 1]);
	}
...
static int gt1x_parse_dts(void)
{
	...
	TS_LOG_INFO("%s: irq_config=%d,algo_id=%d,project_id=%s,config_parm_flag=%d,roi_supported=%d,"
		...
		"tools_support=%d,charger_supported=%d,ic_type=%d,chip_name=%s\n" , __func__,
...
static int gt1x_parse_specific_dts(void)
{
	...
	TS_LOG_INFO("%s: x_max_mt=%d,y_max_mt=%d,panel_id=%d,vendor_name=%s, provide_panel_id_support=%d\n", 
		__func__, ts->max_x, ts->max_y,ts->hw_info.sensor_id,ts->dev_data->module_name,
		ts->dev_data->provide_panel_id_support);
	return NO_ERR;
}
				...
				char *cfg_type, u8 *cfg, int *cfg_len, u8 sid)
{
	...
	if(GT1X_NOT_EXIST == sid_is_exist) {
		...
		strncpy(gt1x_ts->dev_data->ts_platform_data->dsm_info.fw_update_result, "failed", strlen("failed"));
	}
#endif
