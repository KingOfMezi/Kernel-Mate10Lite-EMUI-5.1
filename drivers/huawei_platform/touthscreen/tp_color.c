...
static int read_tp_color_from_nv(char *color_info)
{
	...
	if ((!strncmp(user_info.nv_data, "white", strlen("white")))
		...
		strncpy(color_info, user_info.nv_data,
...
static int write_tp_color_to_nv(void)
{
	...
	case GOLD:
		...
		break;
	case SILVER:
		strncpy(user_info.nv_data, "silver", sizeof(user_info.nv_data));
		break;
	case GRAY:
	...
	case CAFE2:
		...
		break;
	case BLACK2:
		strncpy(user_info.nv_data, "pdblack", sizeof(user_info.nv_data));
		break;
	default:
