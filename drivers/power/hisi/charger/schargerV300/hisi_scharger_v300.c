...
{
    if (dm_vol > HI6523_DPDM_WATER_THRESH_1800MV && dm_vol < HI6523_DPDM_WATER_THRESH_2300MV)
		return TRUE;
    else if (dm_vol > HI6523_DPDM_WATER_THRESH_2890MV && dm_vol < HI6523_DPDM_WATER_THRESH_2920MV)
		return TRUE;
    else
		return FALSE;
}
...
{
    if (dp_vol > HI6523_DPDM_WATER_THRESH_2930MV && dp_vol < HI6523_DPDM_WATER_THRESH_2980MV)
		return TRUE;
    else
		return FALSE;
}
...
int hi6523_is_water_intrused(void)
{
	...
	hi6523_write_mask(CHG_ADC_APPDET_REG, CHG_ADC_APPDET_CHSEL_MSK, CHG_ADC_APPDET_CHSEL_SHIFT, CHG_ADC_APPDET_DMINUS);
...
static int hi6523_scp_set_watchdog_timer(int second)
{
	...
	val |= (second * ONE_BIT_EQUAL_TWO_SECONDS) & SCP_WATCHDOG_MASK; /*1 bit means 0.5 second*/
	SCHARGER_INF("[%s]val after is %d \n", __func__, val);
...
static void hi6523_plugout_check_work(struct work_struct *work)
{
	...
	struct hi6523_device_info *di = g_hi6523_dev;
	static unsigned int vbatt_full=0;
...
static void parse_dts(struct device_node *np, struct hi6523_device_info *di)
{
	...
	ret =of_property_read_u32(np, "weak_source_recovery_en",
				 (u32 *)&weak_source_recovery_en);
	if (ret) {
