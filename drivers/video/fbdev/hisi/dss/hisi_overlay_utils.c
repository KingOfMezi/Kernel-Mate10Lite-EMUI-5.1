...
{
	BUG_ON(dss_base == NULL); //lint !e730
	...
	dss_layer_t *layer, dss_wb_layer_t *wb_layer, int ovl_idx, dss_rect_t *wb_ov_block_rect, bool has_base)
{
	...
	if (wb_layer) {
		...
		if (layer->need_cap & (CAP_DIM | CAP_PURE_COLOR)) {
			...
			if ((g_dss_version_tag == FB_ACCEL_KIRIN970) || (chn_idx != DSS_RCHN_V2)) {
				if (g_dss_version_tag == FB_ACCEL_KIRIN970) {
					if (hisifd->index == MEDIACOMMON_PANEL_IDX) {
	  ...
	  dss_overlay_block_t *pov_h_block, dss_rect_t *wb_ov_block_rect, int ovl_idx, int ov_h_block_idx)
{
	...
	ovl_bg_color = (g_dss_version_tag == FB_ACCEL_KIRIN970) ? 0x00000000 : 0xFF000000;
	...
	dss_layer_t *layer, dss_rect_t *wb_ov_block_rect, bool has_base)
{
	...
	if ((g_dss_version_tag == FB_ACCEL_HI366x) || (g_dss_version_tag == FB_ACCEL_KIRIN970_ES)) {
	...
	dss_overlay_t *pov_req, uint32_t cmdlist_pre_idxs, bool enable_cmdlist, bool *use_comm_mmbuf)
{
	...
	for (m = 0; m < pov_req->ov_block_nums; m++) {
		...
		for (i = 0; i < pov_h_block->layer_nums; i++) {
			...
			if ((chn_idx == DSS_RCHN_V2) && ((g_dss_version_tag == FB_ACCEL_KIRIN970_ES) ||
				(g_dss_version_tag == FB_ACCEL_HI366x))) {  //chicago & boston es copybit
				mctl_idx = DSS_MCTL5;
				has_ovl = false;
			} else {
				mctl_idx = ovl_idx;
	...
	bool enable_cmdlist)
{
	...
	if (((g_dss_version_tag == FB_ACCEL_HI366x) || (g_dss_version_tag == FB_ACCEL_KIRIN970_ES)) &&
		(layer->chn_idx == DSS_RCHN_V2)) {   //copybit
		mctl_idx = DSS_MCTL5;
	} else {
...
int hisi_overlay_init(struct hisi_fb_data_type *hisifd)
{
	...
	if (g_dss_version_tag == FB_ACCEL_HI625x || g_dss_version_tag == FB_ACCEL_HI366x ) {
		hisifd->video_opt_support = is_mipi_video_panel(hisifd) ? true : false;
	} else {
		hisifd->video_opt_support = false;
	}
