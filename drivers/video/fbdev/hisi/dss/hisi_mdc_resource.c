	...
	for (i = 0; i < mdc_ops->chan_num; i++) {
		...
		if ((chn_info->rch_need_cap & CAP_HFBCD) == CAP_HFBCD) {
			if ((chn_info->mmbuf_size == 0)
				|| (chn_info->mmbuf_size > MMBUF_SIZE_MAX)
			...
			chn_info->mmbuf_addr = hisi_dss_mmbuf_alloc(hisifd->mmbuf_gen_pool,
					chn_info->mmbuf_size);
			if (chn_info->mmbuf_addr < MMBUF_BASE) {
				HISI_FB_ERR("fb%d, mmbuf addr is invalid, mmbuf_addr = %d!\n",
					hisifd->index, chn_info->mmbuf_addr);
				return -EINVAL;
			}
		}
	...
	for (i = 0; i < mdc_ops->chan_num; i++) {
		...
		if ((chn_info->rch_need_cap & CAP_HFBCD) == CAP_HFBCD) {
			...
			hisi_dss_mmbuf_free(hisifd->mmbuf_gen_pool,
					chn_info->mmbuf_addr, chn_info->mmbuf_size);
		}
