...
int hisifb_xnit_set(struct fb_info *info, void __user *argp)
{
	int ret = 0;
	unsigned int xnit_value[3];
	...
	ret = (int)copy_from_user(xnit_value, argp, sizeof(xnit_value));
	if (ret) {
		HISI_FB_ERR("copy_from_user(param) failed! ret=%d.\n", ret);
		return -2;
	}

	dpe_store_xnit_Value(xnit_value);
	if (pinfo->xcc_support) {
		if (pinfo->xcc_set_in_isr_support) {
			hisifd->xcc_coef_set = 1;
		} else {
			ret = dpe_set_xcc_cscValue(hisifd);
			if (ret) {
				HISI_FB_ERR("dpe_set_xcc_cscValue failed! ret=%d.\n", ret);
			}
		}
