...
static unsigned int g_xnit_value[3] = {32768, 32768, 32768};
	...
	if (pinfo->xcc_support == 1) {
		...
		if (pinfo->xcc_table_len > 0 && pinfo->xcc_table) {
			...
			outp32(lcp_base + LCP_XCC_COEF_01, pinfo->xcc_table[1]
				* g_led_rg_csc_value[0] / 32768 * color_temp_rectify_R / 32768 * g_xnit_value[0] / 32768);
			...
			outp32(lcp_base + LCP_XCC_COEF_12, pinfo->xcc_table[6]
				* g_led_rg_csc_value[4] / 32768 * color_temp_rectify_G / 32768 * g_xnit_value[1] / 32768);
			...
			outp32(lcp_base + LCP_XCC_COEF_23, pinfo->xcc_table[11]
				...
				* color_temp_rectify_B / 32768 * g_xnit_value[2] / 32768);
		}
...
void dpe_init_xnit_Value(void)
{
	g_xnit_value[0] = 32768;
	g_xnit_value[1] = 32768;
	g_xnit_value[2] = 32768;
	return;
}

void dpe_store_xnit_Value(unsigned int xnit_value[])
{
	g_xnit_value[0] = xnit_value[0];
	g_xnit_value[1] = xnit_value[1];
	g_xnit_value[2] = xnit_value[2];
	HISI_FB_ERR("XNIT: %d %d %d \n", g_xnit_value[0], g_xnit_value[1], g_xnit_value[2]);
	return;
}
