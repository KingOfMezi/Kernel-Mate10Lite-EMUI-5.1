...
bool need_balance_dirty_type(struct f2fs_sb_info *sbi)
{
	...
	unsigned int dirty_node, dirty_data, all_dirties;
	int i;
	unsigned int randnum;
	unsigned long inc_node, inc_data;
	struct timespec ts = {DEF_DIRTY_STAT_INTERVAL, 0};
	unsigned long interval = timespec_to_jiffies(&ts);
	unsigned long last_jiffies;
	bool ret;

	mutex_lock(&sbi->ssr_mutex);
	last_jiffies = sbi->last_ssr_jiffies;
	mutex_unlock(&sbi->ssr_mutex);

	/*lint -save -e550*/
	ret = time_after(jiffies, last_jiffies + interval);
	/*lint -restore*/
	if (!ret)
		return false;
	...
	/* How many blocks are cost during this interval */
	mutex_lock(&sbi->ssr_mutex);
	inc_node = sbi->curr_node_cnt - sbi->last_node_cnt;
	inc_data = sbi->curr_data_cnt - sbi->last_data_cnt;
	sbi->last_node_cnt = sbi->curr_node_cnt;
	sbi->last_data_cnt = sbi->curr_data_cnt;
	sbi->last_ssr_jiffies = jiffies;
	mutex_unlock(&sbi->ssr_mutex);

	if (!all_dirties)
		return false;
	if (dirty_node < (unsigned int)reserved_sections(sbi) &&
					inc_node > sbi->blocks_per_seg) {
		get_random_bytes(&randnum, (int)sizeof(unsigned int));
		if (randnum % 100 > dirty_node * 100 / all_dirties)
			return true;
	}
	if (dirty_data < (unsigned int)reserved_sections(sbi) &&
					inc_data > sbi->blocks_per_seg) {
		get_random_bytes(&randnum, (int)sizeof(unsigned int));
		if (randnum % 100 > dirty_data * 100 / all_dirties)
			return true;
	}
						...
						int type, bool force)
{
	...
	else if (!is_set_ckpt_flags(F2FS_CKPT(sbi), CP_CRC_RECOVERY_FLAG) &&
		...
		struct f2fs_summary *sum, int type)
{
	...
	*new_blkaddr = NEXT_FREE_BLKADDR(sbi, curseg);
	...
	if (type >= CURSEG_HOT_NODE && type <= CURSEG_COLD_NODE)
		sbi->curr_node_cnt++;
	else if (type >= CURSEG_HOT_DATA && type <= CURSEG_COLD_DATA)
		sbi->curr_data_cnt++;
	mutex_unlock(&sbi->ssr_mutex);
